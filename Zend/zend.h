...
typedef union _zvalue_value {
	...
	zend_object_value obj; //一个zend_object_value包含俩个成员, 一个是标识符(整形序号), 表明了当前对象存储在全局对象列表的位置, 另外还有一个zend_object_handlers指针, 指向当前对象所属类的handlers(标准操作集合).
} zvalue_value;
...
struct _zval_struct {
	...
	zend_uchar is_ref__gc; //is_ref = 1 引用变量 is_ref=0非引用变量
};
